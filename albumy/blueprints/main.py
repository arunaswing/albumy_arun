    ...
    if request.method == 'POST' and 'file' in request.files:
        ...
        if user_description:
            ...
            try:
                ...
                if description_result.caption is not None:
                    description = description_result.caption.text
                else:
                    description = ""  
            except Exception as e:
                flash(f"Failed to generate ML description: {str(e)}")
                description = ""  
        ...
        try:
            ...
            if tags_result.objects is not None:
                ml_generated_tags = [tag.tags[0].name for tag in tags_result.objects.list]
                for name in ml_generated_tags:
            ...
            flash(f"Failed to generate ML tags: {str(e)}")
        ...
        if form.submit.data:  
            ...
            try:
                with open(image_path, "rb") as image_file:
                    ...
                    if description_result.caption is not None:
                        ml_generated_description = description_result.caption.text
                        photo.description = ml_generated_description
                        db.session.commit()
                        flash('ML-Generated description added.', 'success')
                    else:
                        flash(f"No ML description available")
            except Exception as e:
                flash(f"Failed to generate ML description: {str(e)}", "danger")
                    ...
                    if tags_result.objects is not None:
                        ...
                        for name in ml_generated_tags:
                            ...
                            if tag not in photo.tags:
                                photo.tags.append(tag)
                                db.session.commit()
                        flash('ML-Generated tags added.', 'success')
            except Exception as e:
                flash(f"Failed to generate ML tags", "danger")
